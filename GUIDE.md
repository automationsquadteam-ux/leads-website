...
| 0046 | `20260913120000_native_language_outreach.sql` | ✅ yes ,**confirmed live 2026-09-13 by functional probe**: the `inherit_initial_approval` trigger approved 253 native initials in the first n8n batch, which only happens if the column, table, function and trigger all exist. Bundle was `supabase/schema-update-36-native-language-outreach.sql`. Adds `email_versions.language` (default `en`) and `.angle`, the `country_languages` table (27 rows), `language_for_country()`, the `leads_needing_native_initial` view n8n reads (service-role grants), two appended columns on `lead_send_queue`, the `inherit_initial_approval` BEFORE INSERT trigger, and the `outreach.require_native_language` setting. **Paste this BEFORE running the n8n native-language batch** ,its insert writes `language`, which does not exist until then. Probe: `select language from email_versions limit 1` returns without "column does not exist"; `select count(*) from country_languages` = 27; `select count(*) from leads_needing_native_initial` ≈ 369 before the batch and falls as it runs; `select value from settings where key = 'outreach.require_native_language'` = `true` |
| 0047 | `20260913180000_inherit_approval_content_check.sql` | ✅ yes ,**confirmed live 2026-09-14 by functional probe**: a throwaway initial containing `[Business Name]`, in the lead's target language, for a lead with an approved initial, landed `status = draft` (0046 alone would have approved it). Bundle was `supabase/schema-update-37-inherit-approval-content-check.sql`. Redefines `inherit_initial_approval()` (0046) so a native initial only inherits approval when its subject+content carry no `[placeholder]`, its content is in its language's script (el/bg/ar/ja), and its opening 250 chars are not an English opener. **Paste BEFORE re-running the n8n batch for the 145 leads now in `leads_needing_native_initial`** ,without it the 34 quarantined kinds of row would be re-approved on sight. Probe: insert a throwaway initial with content `Hola [Business Name]`, `language` matching the lead's country, for a lead that has an approved initial ,it must land `status = 'draft'`; delete it after |

**0001 through 0039, 0042 and 0043 are applied; 0040, 0041, 0044 and 0045 are not yet pasted; 0046 and 0047 are applied.** This table had drifted stale more than once by this point
...
the migration table above). **The next migration after that is 0048.** Add the file, regenerate
...
**A third addition (2026-09-20): a missing sign-off gets one.** `n8n`'s own prompt hardcodes
"Best regards,\n<brand>" as the model's last line in every language, translating only the
greeting ,but the model skips it often enough to matter (2/26 one afternoon, 5/15 in a
Turkish batch the same day), and nothing before this put one back. `ensureSignOff()` appends
the plain English line whenever the configured from-name is absent anywhere in the body.
Deliberately not translated per language: the project already ships ~46 approved native
drafts ending on an English "Best regards, Team Automation" and treats that as a blemish, not
a wrong-language email (next paragraph); inventing untested translations for the other 19
languages would be worse than the blemish it already tolerates, not better.

**Verifying that fix against live drafts found a second, unrelated bug in `findUnresolvedPlaceholders()`
(0040)**: its bracket rule matched `[A-Z][^\]\n]{0,60}` for the first character, which is ASCII
only ,`[İsim]` (Turkish) and `[Αγαπητέ Πελάτη]` (Greek) start with a letter outside that
range and were invisible to it, including at SEND time, since 0040's send-refusal calls this
same function. One live lead reached `approved` with "Merhaba [İsim]!" still in the body. Now
`\p{Lu}` / `\p{Ll}` (Unicode letter-case properties). A full scan of `email_versions`'s entire
5,474 rows found exactly that one case and confirmed it was never sent.
...
| Date | Change |
| --- | --- |
| 2026-09-25 | **The four Pakistan leads tagged "Japan" were deleted permanently.** Asked for directly ("remove these fully"): Auto Surgeon Workshop, Auto Yard PakWheels Partner Workshop, Evergreen Autos and ZAMS Yokohama Tyres Company Outlet Islamabad, all with Pakistani phone numbers and two with .pk domains, pulled in by the n8n search for "Auto repair shops in Yokohama" matching the tyre brand. None had ever been emailed. Deleted from `leads` directly (the same statement `deleteLeads()` runs; it needs an admin session, so it could not be called from a script); ON DELETE CASCADE removed their 8 email_versions, 4 lead_pipeline and 4 lead_activity rows, verified at zero afterwards. No backup was kept, which matches `deleteLeads()`. **A repeat of that n8n search could bring them back.** Data only |
| 2026-09-25 | **164 native-language initials written by hand, clearing the language-hold backlog.** Asked for directly because n8n's native-draft workflow is not working: every lead in the send pool held by the 0046 hold (approved English initial, wrong language for its country) got a native initial written from its approved English one, keeping each email's specific observations and offer. Inserted through `createEmailVersion()` as `draft` with `language` and a native `angle`, `generated_by = 'claude:native'`, plus a `draft_regenerated` activity row, so the 0046/0047 trigger decided approval exactly as for an n8n insert: **164/164 inherited approval and left the hold** (de 32, ar 22, nl 21, no 18, sv 17, da 15, ms 14, id 14, fr 11). Every draft was checked beforehand against 0047's rules (no `[placeholder]`, script for ar, no English opening) and afterwards against the real send-time `findUnresolvedPlaceholders()`: 0 would be refused. Conventions used: formal register (Sie/u/vous/Bapak-Ibu, plural dere/I/ni/كم), a native sign-off before "Team Automation" instead of the English "Best regards", Swiss spelling (ss) for the Zurich leads, Malaysian Malay rather than the Indonesian the n8n Malay drafts were really in, and law-firm chatbots that route to a lawyer rather than give legal guidance. **The approved English sources were worse than expected**, and the native versions do not repeat their errors: live placeholders in three approved drafts (KS-Kraftwagenservice `[something specific about the business]`, Hypoteket `[Mortgage broker]`, Arctic Friend `[insert specific example from the website]`), invented statistics (Legalis "30% decrease"), "we've helped numerous firms" claims, a Danish draft for a Norwegian firm, raw English research pasted into an Indonesian draft, guessed first names, and wrong niches (Danish travel agencies and VisitDenmark tagged "insurance agencies", Vienna PR and creative agencies tagged "recruitment", Lannebo Fonder as a "mortgage broker"). **Deliberately not done:** the four "Japan" leads (Auto Surgeon Workshop, Auto Yard PakWheels, Evergreen Autos, ZAMS Yokohama Tyres Islamabad) are in Pakistan (+92 numbers, .pk domains; the n8n search for "Auto repair shops in Yokohama" matched the tyre brand), so no Japanese was written and they stay held; their approved English drafts also say "Yokohama, Japan". The sender fix from the entry below was confirmed live the same evening: 40/40 at 20:39 PKT, with ONE SHIPPING LDA moved to Send Failures as `not_approved`. Data only |
| 2026-09-25 | **The daily cap stopped at 39 because one held lead blocked the last slot; unapproved initials now go to Send Failures instead.** 2026-09-12 and 2026-09-25 each ended at 39/40, then "Nothing is due" every 3 minutes until midnight. `findDueWork()` fetched exactly `limit` initial rows and filtered afterwards, so with one send left it fetched ONE row: ONE SHIPPING LDA, #1 in the queue on an English approval from 16 Aug, whose active initial is a Portuguese **draft** that failed 0047's content check (opens in English). Dropped, nothing left, same row next run. It sat in the queue at all because `sync_pipeline_from_version()` ORs `lead_pipeline.approved`: a newer unapproved draft replacing an approved one never turns the flag off (rejecting on the lead page does, via `review.ts`). It was the only such lead among 639 queue candidates; 470 were sendable and 168 are language holds. **Fixes, code only.** (1) The initial branch pages through `lead_send_queue` (100 at a time, `lead_id` tie-breaker for stable paging) until it has enough sendable leads; follow-up queries over-fetch by the number of blocked leads. Open failures are loaded once up front instead of filtered at the end. (2) An initial the sender walks past with an unapproved active draft gets a Send Failures row, new reason **`not_approved`** ("Draft not approved"), saying which language and why it was not auto-approved (failed content check, English never approved, or nobody approved it). The open failure keeps it out of later runs. (3) **Approving the draft clears that entry by itself**, the one exception to Mark fixed: `clearApprovedDraftFailures()` runs at the top of every non-dry cron run (catches the sweep and the n8n inheritance trigger) and in both approve actions. (4) The hourly draft sweep no longer auto-approves a translation when the lead has an English initial and none of them was approved (new blocking issue `english_not_approved`). It had approved 18 that way (14 rejected English, 4 never reviewed; 17 Turkey/Kuwait/Saudi/Hungary, 1 Germany). **Data:** those 18 were sent back to draft with a review note, `sweep_checked_at` set so no sweep re-approves them, and `lead_pipeline.approved` switched off, the same way 0022 did it; 5 are now in the approval queue (stage `review`), the other 13 need an address or verification first. None had been sent. Verified live against the real code: with 1 slot left it now picks Complete Physio and would move ONE SHIPPING to Send Failures; with 25 it pages past all 168 holds; the dry run wrote nothing; the clear deletes only for a lead whose active initial is approved. **Parked by the operator, not changed:** the 168 language holds (32 de, 22 ar, 21 nl, 18 no, 17 sv, 15 da, 14 ms, 14 id, 11 fr, 4 ja) stay quiet holds because n8n is not working properly; they are to be rewritten another way later. Three more native drafts behind an earlier approval (Richel Bouw, Brodin Shipping, ZR Zurich Relocation) carry leftover placeholders but have unverified addresses, so they are not in the send pool yet |
| 2026-09-23 | **The catch-all/unknown re-check download no longer also re-sends never-checked addresses, and the two verdicts are grouped instead of interleaved.** `?recheck=1` on `/api/admin/emails/unverified.csv` had been returning `unverified` + `unknown` + `accept_all` together in lead-id order, even though the button reads "Also re-check catch-all & unknown" ,so every never-checked address was quietly billed a second time inside that file. `getUnverifiedEmails({ includeInconclusive: true })` now returns only `unknown` and `accept_all`; `buildUnverifiedCsv()` groups them into all `unknown` rows, one blank line, then all `accept_all` rows, and skips a group's blank line entirely when only one of the two is present. The Settings button's count now reads `inconclusive` alone rather than `exportable + inconclusive`. Code only |
| 2026-09-20 | **A missing sign-off is now repaired, not just noticed ,and finding that exposed a real send-safety gap.** Asked directly after the day's native batch dropped "Best regards, Team Automation" on 2/26 English and 5/15 Turkish initials. `repairDraft()` now appends `Best regards,\n<fromName>` (English, deliberately ,see the "known and accepted" note below) whenever the brand name is absent anywhere in the body; live-tested against all 8 real drafts missing one plus 32 that already had one, zero false positives. **Verifying it live surfaced a real bug, not caused by this change:** `findUnresolvedPlaceholders()`'s bracket rule required an ASCII `[A-Z]` as a placeholder's first letter, so `[İsim]` (Turkish) and `[Αγαπητέ Πελάτη]` (Greek) were invisible to it ,including at SEND time, since this is the same function 0040's unconditional guard calls. One live lead ("İstanbul Çatı Ustası-Kiremit Aktarma…") had reached `approved` with "Merhaba [İsim]!" still in it. Fixed to `\p{Lu}` / `\p{Ll}` (Unicode letter properties); a full scan of all 5,474 rows in `email_versions` found exactly that one case, confirms **zero were ever sent**, and it is rejected and reset the same way as the two reset batches below. **Not done, by choice:** the salutation-collapse rule that turns "Hi [Owner's Name]," into "Hi," only recognises English greeting words, so "Merhaba [İsim]!" will not self-heal the way an English one would ,left for a human or a re-run rather than guessing at a Turkish equivalent unasked. Code + one lead's data |
| 2026-09-20 | **The country wasn't reaching the n8n language node; confirmed fixed against a live re-run.** The 2026-09-19 reset 9 (llama3.2:3b batch) came back clean under llama3.1:8b, but a NEW batch of 15 Turkish leads landed with English bodies tagged `en` ,the research+draft workflow's language step wasn't receiving `country`. Traced with the operator to the exact node; fixed on the n8n side. Reset the 15 the same way (version rejected kept in history, research fields + `researched_at` cleared, `research_complete`/`draft_ready`/`approved` forced false ,none of those flip back on their own). Re-run confirmed **15/15 came back genuinely in Turkish**, correctly tagged `tr`; across everything n8n wrote that day (40 active initials), zero language mismatches remained. Data only |
...
| 2026-09-13 | **A successful send now clears its own prior failures, and "Mark fixed" is on the lead page too ,because the block-until-fixed gate had a false answer built in.** Reported as "I fixed it and sent the initial but it still says not fixed", and the live rows made the mechanism exact: Manpower Norge `failed` at 12:09 on 2026-08-26 (the trailing-dot address), the address was corrected, and the initial went out `sent` at 12:12 ,three minutes later. The 12:09 row stayed, the gate reads "any failed row?", so the lead sat blocked for its follow-ups for weeks over a problem the very next send had already proved was gone. **The provider accepting a message is stronger evidence a cause is resolved than a human clicking a button**, so `sendLeadEmail()` now deletes that lead's failure rows older than the send on every success (only older ones: this attempt's own row is never a failure at that point, and a failure landing after this instant is a new problem). The gate condition stays "any failed row" ,it did not need to get cleverer, it needed the rows to leave when they should. **The button stays for the one case this cannot cover: a fix not yet followed by a send.** `ClearFailureButton` moved from the send-failures route folder to `components/`, and the lead page now shows a "Blocked from sending" notice beside the pipeline tracker with the reason and the button ,fixing an address on this page and then hunting for a different page to release it was the last step of one job split across two screens. Derived from the `emailLogs` `getLeadDetail()` already loads, no new query. **One-time:** applied the new rule retroactively to Manpower Norge (the only lead in that state, verified by sweeping every lead with a failure row against its newest successful send); it is released. Code only, no migration |
| 2026-08-28 | **Email Schedule: each of the seven past-day rows now opens that day in the Email Logs.** Links to `/email-logs?from=YYYY-MM-DD` with `from` and deliberately NO `to` ,`resolveDateRange()` in `lib/data/misc.ts` reads a single bound as "that one day", and the logs page words its own heading off the same distinction (`from && to ? 'period' : 'day'`), so sending both bounds would have it announce a "period" covering one date. `ScheduleDay.date` is already `YYYY-MM-DD` in DISPLAY_TIME_ZONE, which is exactly what that filter's `/^\d{4}-\d{2}-\d{2}$/` accepts, so no formatting sits in between. Uses the same stretched-link pattern as the Send Failures table (anchor in one cell, `absolute inset-0` against the row's `relative`) because a `<tr>` cannot be wrapped in an `<a>` ,the parser hoists it straight back out ,plus `overflow-visible` on that cell, since `TD` clips by default and would otherwise shrink the hit area to the date alone. Only the PAST table is clickable; the forecast rows describe sends that have not happened and have no log to open. Heading carries a "click a day to open its email log" hint, because a clickable table row advertises nothing on its own |
