-- ===========================================================================
-- Schema update 18 - a verdict belongs to an ADDRESS, and so does identity.
--
-- GENERATED FILE. Regenerate from supabase/migrations/, do not hand-edit.
-- Apply updates 1-17 first. Re-runnable throughout.
--
-- Fixes two bugs with one root cause - changing leads.email left everything
-- that was true of the OLD address attached to the lead:
--
--   1. DUPLICATE LEADS. dedupe_key was never recomputed, so correcting an
--      address made the next sheet sync insert a second lead for the same row.
--      Eight sheet rows in the live data are currently claimed by two leads.
--   2. TRANSFERRED VERDICTS. A NeverBounce result followed the lead onto a
--      different address: 'valid' meant an unchecked address passed the send
--      gate, 'invalid' meant a corrected typo stayed blocked for ever.
--
-- Also adds send priority, so initial emails go out verifier-proved first and
-- hand-confirmed second. Ordering only - nothing is gated.
-- ===========================================================================
