-- ===========================================================================
-- Schema update 32 - follow-up due dates land on a whole calendar day.
--
-- GENERATED FILE. Regenerate from supabase/migrations/, do not hand-edit.
-- Apply updates 1-31 first. Re-runnable throughout.
--
-- followup1_due/followup2_due used to be sent_at + N days - exact timestamp
-- arithmetic, so a follow-up became due at whatever MINUTE the previous send
-- happened to land on, N days later. Both functions that compute these
-- (sync_pipeline_from_email_log for this app's own sends,
-- sync_pipeline_from_lead for n8n-reported sends) now truncate to the
-- calendar day first, landing on midnight (Asia/Karachi) of the Nth day.
-- NOT retroactive - only changes the next time each column gets set; nothing
