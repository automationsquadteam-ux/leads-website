-- ===========================================================================
-- Schema update 25 - the scheduled sender could not see a single lead.
--
-- GENERATED FILE. Regenerate from supabase/migrations/, do not hand-edit.
-- Apply updates 1-24 first. Re-runnable throughout.
--
-- URGENT FIX. Since 0028 the initial-send query read `pipeline_board`, whose
-- body ends in `where public.is_admin()`. The scheduler runs on the
-- service-role key, which satisfies no such predicate, so the view returned
-- ZERO ROWS on every run and no initial email has auto-sent since. Adds
-- `lead_send_queue`, protected by grants instead of a predicate.
-- ===========================================================================
