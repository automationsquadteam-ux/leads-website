-- ===========================================================================
-- Schema update 15 - the stage names what is BLOCKING a lead, plus cleanup.
--
-- GENERATED FILE. Regenerate from supabase/migrations/, do not hand-edit.
-- Apply updates 1-14 first. Re-runnable: every drop is `if exists`, every
-- function is `create or replace`, and the stage recompute is a no-op UPDATE.
--
-- Paste into the Supabase SQL editor and Run.
--
-- What it does:
--   1. compute_pipeline_stage / compute_next_step reordered to FIRST UNMET
--      GATE, so a stage answers "what is stopping this lead" instead of "what
--      was the last thing that got done". 497 leads move backwards into
--      need_email / need_verification. Nothing is destroyed.
--   2. A verification result now decides email_verified outright: verified
--      means 'valid' and nothing else. Moving a lead to Catch-all or Unknown
--      used to leave the flag true.
--   3. sync_pipeline_from_lead() stops deriving `approved` from leads.status.
--      It KEEPS reading 'sent'/'replied', which is the only record of the 89
--      sends made upstream.
--   4. Campaigns and templates removed entirely - every lead has
--      campaign_id = NULL, so the generator never used them.
--   5. Ten views nothing reads are dropped; pipeline_board and
--      dashboard_leads_safe are rebuilt without the dropped columns.
--   6. leads.category, leads.next_followup_at, email_logs.campaign_id and
--      email_logs.template_id dropped; three orphan settings rows deleted.
-- ===========================================================================
