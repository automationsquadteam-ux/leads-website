-- ===========================================================================
-- Schema update 36 - outreach in the lead's own language.
--
-- GENERATED FILE. Regenerate from supabase/migrations/, do not hand-edit.
-- Apply updates 1-35 first. Re-runnable throughout.
--
-- See supabase/migrations/20260913120000_native_language_outreach.sql for
-- the full reasoning. Adds email_versions.language / .angle, the
-- country_languages table, the leads_needing_native_initial view n8n reads,
-- two columns on lead_send_queue, an approval-inheritance trigger, and the
-- outreach.require_native_language setting.
--
-- PROBE (do not mark 0046 applied on the strength of pasting it):
--   select language from email_versions limit 1;             -- no "column does not exist"
--   select count(*) from country_languages;                   -- 27
--   select count(*) from leads_needing_native_initial;        -- ~369 before the n8n batch, falling as it runs
--   select target_language, initial_language from lead_send_queue limit 1;
--   select value from settings where key = 'outreach.require_native_language';  -- true
-- ===========================================================================
