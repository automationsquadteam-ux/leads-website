-- ===========================================================================
-- Schema update 5 — email verification, draft-version repair, public leads.
--
-- GENERATED FILE. Regenerate from supabase/migrations/, do not hand-edit.
--
-- Apply updates 1-4 first. Paste this into the Supabase SQL editor and Run.
-- Fully re-runnable: create-or-replace throughout, ADD COLUMN IF NOT EXISTS,
-- ON CONFLICT DO NOTHING, and both backfills guarded by NOT EXISTS / IS NULL.
-- Safe to run again after a partial or rolled-back attempt.
--
-- Fixes, in order of how much they were hurting:
--   * leads holding a draft with no email_versions row (the review screen said
--     "no draft" for leads that had one) -- now a trigger, not a one-off
--   * leads the sheet reports as sent whose pipeline never recorded the send
--   * analytics_industry_performance counting email_logs (CRM sends only)
--     instead of lead_pipeline (was this lead emailed, by anyone)
--
-- Adds: email verification state, and an OFF-BY-DEFAULT public lead list.
-- ===========================================================================
-- ---------------------------------------------------------------------------
