-- ===========================================================================
-- Schema update 9 - the sheet's "Date Sent" anchors the follow-up schedule.
--
-- GENERATED FILE. Regenerate from supabase/migrations/, do not hand-edit.
-- Apply updates 1-8 first. Re-runnable: the UPDATE is a no-op once applied.
--
-- Update 5 backfilled first_email_sent from imported_at because Date Sent was
-- empty at the time. Now that it is filled in, the sync could not use it: the
-- ON CONFLICT clause coalesced and therefore never overwrote. Follow-ups would
-- have fired a week after the IMPORT rather than a week after the real send.
--
-- The rule this sets:
--   * a send this CRM made (email_logs row with sent_at) is authoritative and
--     the sheet may not move it
