-- ===========================================================================
-- Schema update 13 - ticking "Email verified" is a verdict.
--
-- GENERATED FILE. Regenerate from supabase/migrations/, do not hand-edit.
-- Apply updates 1-12 first. Re-runnable: the catch-up UPDATE is guarded.
--
-- The flag and the status were linked one way only: a verifier result set both,
-- but an admin ticking the box on the lead page set only email_verified. The
-- status stayed 'unverified', the leads table kept saying "Never checked", and
-- the lead stayed in the verifier export - being re-billed for an address a
-- human had already confirmed.
--
-- Now bidirectional, and which side wins is decided by which one CHANGED in the
-- statement, so neither silently overwrites the other. An 'invalid' status is
-- never softened by unticking: a hard bounce is evidence.
-- ===========================================================================
-- ---------------------------------------------------------------------------
